...
static char _log_entry_fmt[] = "[%s] - %s\n"; // [level_tag] - message
...
bool is_file_enabled(log_level level) {
...
static bool _log_file(log_level level, const char* fmt_msg, va_list argp) {
...
static bool _log_stderr(log_level level, const char* fmt, va_list argp) {
...
void set_context(log_level level) {
    // nop
}
